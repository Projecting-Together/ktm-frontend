...
export default function ManageVisitsPage() {
  ...
  const { mutate: confirm } = useMutation({
    mutationFn: ({ id, date }: { id: string; date: string }) => confirmVisit(id, date).then(r => { if (r.error) throw new Error(r.error.message); return r.data!; }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["visits", "received"] }); toast.success("Visit confirmed"); },
  ...
  return (
    <div>
      ...
      <p className="text-muted-foreground mb-6">Manage property visit requests from renters.</p>
          ...
          {visits.map((v) => (
            <div key={v.id} className="rounded-xl border border-border bg-card p-5 flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                {v.status === "pending" && (
                  <button onClick={() => confirm({ id: v.id, date: v.preferred_date })} className="btn-primary text-xs px-3 py-1.5">Confirm</button>
                )}
              </div>
            </div>
          ))}

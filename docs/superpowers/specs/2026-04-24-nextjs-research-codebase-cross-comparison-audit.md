...
| Testing and verification readiness | VERIFY-02 | Track and validate stale-window behavior for ISR pages under representative traffic. | Unknown: no ISR stale-window or cache-age assertions were found in the currently tracked branch test suite. | Unknown | Medium | Medium | Medium | Requires adding a dedicated ISR freshness/performance suite with explicit stale-window assertions. |
...
- Validate command: `npx playwright test --config playwright.perf.config.ts tests/performance`
- Expected behavior: Once the perf suite and config are added, all stale-window/freshness specs pass and fail on regression beyond defined thresholds.

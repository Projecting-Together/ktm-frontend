...
- Wave 1 scalability program: `docs/superpowers/specs/2026-04-24-frontend-scalability-maintainability-program-design.md`
...
## Public listing cache (Wave 1)

Public listing reads for home and detail use Next `fetch` with **tags** in `src/lib/api/server-public-listings.ts`. After mutations, `revalidatePublicListingCache` in `src/lib/cache/revalidate-public-listings.ts` calls `revalidateTag`. See `route-cache-intent.md` for the route matrix and tag table.

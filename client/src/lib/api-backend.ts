...
// Get backend URL from environment or use exposed backend URL
const getBackendURL = () => {
  ...
  // For development, use the exposed backend URL
  if (typeof window !== 'undefined') {
    const hostname = window.location.hostname;
    // Replace 3000 port prefix with 8000 port prefix
    const backendDomain = hostname.replace(/^3000-/, '8000-');

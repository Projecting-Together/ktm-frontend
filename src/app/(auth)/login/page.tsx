...
import { resolvePostLoginRedirect } from "@/lib/constants/memberDashboardRoutes";

function LoginForm() {
  const [showPw, setShowPw] = useState(false);
  const searchParams = useSearchParams();
  const redirectAfterLogin = resolvePostLoginRedirect(searchParams.get("next"));
      ...
      <div className="w-full max-w-md">
        <div className="mb-8 text-center">
          <Link href="/" className="inline-flex items-center gap-2 font-bold text-primary">
            ...
            <span className="text-xl" style={{ fontFamily: "var(--font-serif), Georgia, serif" }}>
        ...
        <div className="rounded-2xl border border-border bg-card p-8 shadow-sm">
          <form onSubmit={handleSubmit((data) => login(data))} className="space-y-5">
            ...
            <button
              ...
              disabled
              aria-disabled="true"
              className="h-11 w-full cursor-not-allowed rounded-lg border border-border bg-background text-sm font-medium text-muted-foreground opacity-70"
            >

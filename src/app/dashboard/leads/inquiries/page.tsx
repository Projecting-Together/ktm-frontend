...
export default function LeadInboxInquiriesPage() {
  ...
  const { mutate: sendReply } = useMutation({
    mutationFn: ({ id, reply }: { id: string; reply: string }) =>
      replyToInquiry(id, reply).then((r) => {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-1">Lead inbox</h1>
      <p className="text-muted-foreground mb-6">Inquiries renters sent about your listings — reply here.</p>
                ...
                (replyingTo === inq.id ? (
                  <div className="mt-3 space-y-2">
                    <textarea
                      ...
                      onChange={(e) => setReplyText((p) => ({ ...p, [inq.id]: e.target.value }))}
                    ...
                    <div className="flex gap-2">
                      <button
                        type="button"
                        onClick={() => sendReply({ id: inq.id, reply: replyText[inq.id] ?? "" })}
                        className="btn-primary text-xs px-4 py-2"
                      >
                        Send Reply
                      </button>
                      <button type="button" onClick={() => setReplyingTo(null)} className="btn-secondary text-xs px-4 py-2">
                        Cancel
                      </button>
                    </div>
                  </div>
                ) : (
                  <button type="button" onClick={() => setReplyingTo(inq.id)} className="mt-3 text-xs font-medium text-accent hover:underline">
                    Reply
                  </button>
                ))}
            </div>
